...
通过以上配置，每当 `push` 新内容的时候，就会在 github 仓库触发这个名叫 `blog deploy pipline` 的 `workflow` ，这个 `workflow` 会将执行编译操作，并将编译之后的文件部署在 `gh-pages` 这个分支。
...
写作最重要的便是找到一款功能垂直，颜值过关，的编辑器。在尝试了几乎所有 `markdown` 编辑器之后，终于找到一款能够满足几乎说有需求的工具。
...
需要注意的是，由于 Obsidian 对 markdown 语法进行了很多丰富，因此我们在使用 Obsidian 进行创作的时候，一些特殊语法是不能使用的，否则 hugo 的 markdown 解释器无法解析相应的语法。
